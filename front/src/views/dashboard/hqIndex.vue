    ...
    <div class="home_session">
      <div class="home_left_block">
        ...
        <div>
          ...
          ><span class="icon_block" /><span class="echart_titile">每日各时段营业额走势
        ...
        <div v-loading="shopDetail">
          <div class="echart_head head_line"><span class="icon_block" />
            <span class="echart_titile">本月营业额门店对比</span>
            <div class="fr data_btn">
              <el-button
                :class="shopActive==1 ? 'active_data':''"
                size="mini"
                round
                @click="changeShopMonthBussiness(1)"
              >总额</el-button>
              <el-button
                :class="shopActive===0 ? 'active_data':''"
                size="mini"
                round
                @click="changeShopMonthBussiness(0)"
              >门票</el-button>
              <el-button
                :class="shopActive===2 ? 'active_data':''"
                size="mini"
                round
                @click="changeShopMonthBussiness(2)"
              >会员卡</el-button>
              <el-button
                :class="shopActive===3 ? 'active_data':''"
                size="mini"
                round
                @click="changeShopMonthBussiness(3)"
              >充值套餐</el-button>
            </div>
          </div>
          <barChart :data-arrary="allShopDetail" />
        </div>
      </div>
      <div class="home_right_block">
        ...
        <!-- <div class="right_items attention">
          ...
          <div class="attention_item">
            ...
            >充值套餐</el-button>
            <span class="attention_content">有{{ storedExpireNum }}件充值套餐即将到达使用有效期</span>
          </div>
...
import { queryHqShopMonthBusiness, queryExpireGoods, queryDayTrendChart, queryMonthPayBusiness, queryMonthTrendChart, querybusinessStatView, queryShopSurveyStat } from '@/api/hq/homePage'
export default {
  components: {
    PieChart,
    lineMarker,
  ...
  methods: {
    ...
    changeShopMonthBussiness(val) {
      if (this.allShopDetail.length >= 1) {
        this.allShopDetail = []
      }
      this.shopActive = val
      var shopKey = null
      ...
      if (val === 3) { // 会员卡
        shopKey = 'storedAmount'
      }

      this.allShopData.forEach(v => {
        const data = {
          name: v.shopName,
          value: v[shopKey]
        }
        this.allShopDetail.push(data)
      })
    },
    /* 获取所有门店 */
    getStoreList() {
      ...
      queryHqShopList(params).then(res => {
        if (res.errorCode === 9000) {
          ...
          res.results.forEach(e => {
            if (e.name !== '总部门店' && e.shopStatus == 1) { //eslint-disable-line
              shopIds.push(e.id)
            }
          })
          const data = {
          ...
          queryHqShopMonthBusiness(data).then(res => {
            if (res.errorCode === 9000) {
              this.allShopData = Object.assign([], res.items)
              res.items.forEach(e => {
                const data = {
                  name: e.shopName,
                  value: e.totalAmount
